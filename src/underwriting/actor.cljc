(ns underwriting.actor
  "LoanUnderwritingActor — the ISCO-08 3312 community underwriting actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one underwriting operation request
  ...
  The unconditional invariant: the LoanUnderwritingAdvisor can never
  directly commit a record the LoanUnderwritingGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [underwriting.advisor :as advisor]
            [underwriting.governor :as governor]
            [underwriting.store :as store]))

(defn build-graph
  "Build a compiled LoanUnderwritingActor graph. `store` implements
  `underwriting.store/Store`. `advisor` implements
  `underwriting.advisor/Advisor` (defaults to `mock-advisor`).
